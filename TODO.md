...
- ✅ ~~**App icons + splash screen**~~ — *shipped in `feat/app-icons-splash-screen`.* `react-native-bootsplash` v6 installed; all icon/splash assets generated from the real 1024×1024 app icon (`#1f2937` slate background, 160dp logo). iOS: `BootSplash.storyboard` + `Colors.xcassets` added, `Info.plist` updated, pbxproj references wired, `AppDelegate.swift` calls `RNBootSplash.initWithStoryboard`. Android: `drawable-*dpi/bootsplash_logo.png` generated, `colors.xml` + `styles.xml` (BootTheme) written, `AndroidManifest.xml` activity uses `@style/BootTheme`, `MainActivity.kt` calls `RNBootSplash.init`. JS: `BootSplash.hide({ fade: true })` fires in `AppContent` once auth + onboarding + version checks have all resolved.
