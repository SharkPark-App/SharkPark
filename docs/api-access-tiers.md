...
| `GET`  | `/api/v1/health`                  | Liveness alias (delegates to `HealthController`) |
| `GET`  | `/api/v1/health/live`             | Liveness probe (Fly/k8s)             |
| `GET`  | `/api/v1/health/ready`            | Readiness probe (Fly/k8s)            |
| `GET`  | `/api/v1/weather/current`         | Current weather (via NWS proxy)      |
| `GET`  | `/api/v1/weather/impact`          | Heuristic weather → occupancy multiplier (no live counts) |
| `GET`  | `/api/v1/events`                  | Campus event calendar                |
| `GET`  | `/api/v1/events/upcoming`         | Calendar filtered to a near-term window (mobile event banner) |
| `GET`  | `/api/v1/reliability/lots`        | Bulk per-lot reliability scores (derived from history) |
| `GET`  | `/api/v1/reliability/lots/:lotId` | Per-lot reliability score            |
| `GET`  | `/api/v1/reliability/config`      | Reliability scoring weights (config, not user data) |
| `GET`  | `/api/v1/occupancy-events/lots/:lotId/stats` | Aggregated per-lot stats (counts/averages, no per-event rows) |
| `GET`  | `/api/v1/occupancy-events/snapshots/:lotId`  | Per-day snapshots (same data class as `/lots/:id/history`) |
...
| `GET`  | `/api/v1/occupancy-events/lots/:lotId`        | Recent raw event stream for a lot (same live signal as `/lots/summary`) |
...
A full controller-by-controller audit was performed on **2026-04-29 / 2026-04-30**. Findings have been folded into the endpoint tables above. For the record, the audit:

- Locked `/lots/:id/history` and `/occupancy-events/snapshots/:lotId` to **Public** (cached aggregates, not a live signal).
- Moved `/occupancy-events/lots/:lotId` from Public to **Contributor** (returns the same live signal `/lots/summary` is gated on).
- Deleted the duplicate `/health` handler in `AppController`; `HealthController` is the sole owner of `/health`, `/health/live`, `/health/ready`.
- Removed `GET /events/:id` and `GET /events/:id/impacts` from the spec (no handler, no mobile caller; events are a display/notification surface, not a forecasting layer).
- Asserted in `apps/backend/test/app.e2e-spec.ts` that anonymous `POST /reports` returns `401` via the global `AzureAdGuard`.

### Decorator-to-tier mapping (reference)

The global guard is `AzureAdGuard` (`app.module.ts`), so **no decorator = Authenticated**.
...
- Mobile-side audit (`apps/mobile/src/services/api/*` → tier per call → guest-mode behavior on 401/403). Tracked in `TODO.md`.
- ACCESS-3 / ACCESS-4 e2e tests. Tracked separately.
- `HmacGuard` is **permissive when `DEVICE_EVENT_SECRET` is unset**. Setting the secret in Fly is on the urgent list.
