...
| **Phone number** | Optional; user-supplied | Not collected in current flows (column reserved) | `users.phone` (Neon) | Until account deletion | None | Yes |
...
| **User-submitted reports** | Report type, optional free-text message, lot, timestamp | In-app form | `reports` table (Neon) | Indefinite (moderation / audit) | None | Yes |
...
| **Notification delivery log** | Notification type, lot or event reference, timestamp | Written on each send | `notification_logs` table (Neon) | Indefinite (dedup window; no prune cron yet) | None | Yes (via user_id FK) |
...
| **Contributor ping** | First/last seen timestamps, optional grant timestamp — keyed by device\_hash | Updated on each geofence POST | `contributor_pings` table (Neon) | No automated prune (rows are tiny; access controlled by TTL in app logic) | None | Pseudonymous |
...
- **`notification_logs` retention** — no prune cron exists yet. Rows are small but will grow indefinitely. Consider adding a 90-day window to match Sentry's crash log retention.
- **`contributor_pings` retention** — no prune cron. Once a device stops contributing its row lives forever. Low risk (pseudonymous), but worth a periodic cleanup.
- **Phone number** — column exists but is not collected in any current flow. If it is ever collected, it must be added to the App Store label under Contact Info → Phone Number.
- **Report free-text** — user-controlled; could contain PII the user types themselves. No automated scrubbing in place.
