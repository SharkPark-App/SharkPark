# SharkPark Infrastructure — Aurora / Lambda Tiered Plan (ARCHIVED, Feb 2026)

> **⚠️ This document is historical.** It describes the AWS-centric tiered architecture
> (Aurora Serverless v2 + Lambda + API Gateway + RDS Proxy) that SharkPark planned in
> February 2026 and rejected before launch. We never deployed Tier 1 — we went
> straight to a different Tier 3 stack on **Fly.io + Neon + Cloudflare R2**.
>
> The current architecture is documented in [`../README.md`](../README.md).
> This file is kept as the architectural decision record (ADR) explaining why we
> picked PostgreSQL over DynamoDB. Everything from "Deployment Tiers" downward
> describes infrastructure we **do not run** — do not use it as a deployment guide.
...
| `occupancy_events` | 30 days | Daily cron `DELETE WHERE timestamp < NOW() - INTERVAL '30 days'` (`apps/backend/src/scheduler/jobs/prune-old-data.job.ts`, override with `RETENTION_DAYS` env). Honors README privacy promise; snapshots already carry the aggregated history needed for ML. |
