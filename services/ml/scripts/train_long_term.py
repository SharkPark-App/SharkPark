...
def train(
    ...
    *,
    ...
    Args:
        data_path: Path or glob pattern for parquet file(s) with synthetic v1 data.
        ...
        hyperparams: Optional XGBoost overrides.
        include_synthetic_v2: Load v2 catalog-driven rows from
            ``synthetic_observations``.
        synthetic_v2_school: School short_name filter for v2 (required when
            ``include_synthetic_v2=True``).
        synthetic_v2_term: Term filter for v2 (e.g. ``"Spring_2026"``).
    ...
    Returns:
        MLflow run ID.
    """
    # Load synthetic v1 data. Tolerate empty glob when v2 is also requested.
