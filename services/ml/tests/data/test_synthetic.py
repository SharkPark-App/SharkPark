...
class TestResolveSemester:
    """Verify semester key parsing and resolution."""
    ...
    def test_intersession_rejected(self):
        """Intersessions are not supported — heuristics assume ~16-week semesters."""
        with pytest.raises(ValueError, match="Unknown term"):
            resolve_semester("winter-2026")

    def test_invalid_term_raises(self):
        with pytest.raises(ValueError, match="Unknown term"):
            resolve_semester("autumn-2025")

    def test_invalid_format_raises(self):
        with pytest.raises(ValueError, match="Invalid semester key"):
            resolve_semester("fall2025")


class TestGenerateAllData:
    """Full dataset generation across all lots. Validates DataFrame schema and content."""

    def test_has_required_columns(self, sample_lots, fall_2025_cfg):
        """
    ...
    def test_all_lots_represented(self, sample_lots, fall_2025_cfg):
        """
    ...
    def test_occupancy_rate_in_valid_range(self, sample_lots, fall_2025_cfg):
        """
    ...
    def test_spring_semester_generates_data(self, sample_lots):
        ...
        cfg = resolve_semester("spring-2026")
        df = generate_all_data(sample_lots, cfg, max_per_lot=50)
        assert len(df) > 0
        assert set(df["lot_id"].unique()) == {"G1", "G2", "E1"}
